...
bool Stage::OnCollisionStageAndRect(RectCollider rect_, float vec_x_, float vec_y_, float* side_x_, float* side_y_)
{
	...
	int minX = (rect_.posX - mapOffSetX) / blockSize;						//‰E•Ó
	int maxX = ((rect_.posX + rect_.width) - mapOffSetX) / blockSize;		//¶•Ó
	int minY = (rect_.posY - mapOffSetY) / blockSize;						//ã•Ó
	int maxY = ((rect_.posY + rect_.height)- mapOffSetY) / blockSize;		//‰º•Ó
